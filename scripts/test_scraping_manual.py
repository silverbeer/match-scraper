    ...
    try:
        ...
        if matches:
            ...
            for i, match in enumerate(matches[:5], 1):  # Show first 5 matches
                ...
                score_text = f" ({match.get_score_string()})" if match.has_score() else ""
                print(f"   {status_emoji} {match.home_team} vs {match.away_team}{score_text}")
...
async def test_different_configurations():
    ...
    for test_config in test_configs:
        ...
        try:
            ...
            print(f"   ✅ Success: {len(matches)} matches, {metrics.games_scored} with scores")
