...
class SupabaseDataFixer:
        ...
        try:
            ...
            if games:
                ...
                for i, game in enumerate(games[:5]):  # Show first 5
                    console.print(
                        f"  {i + 1}. ID: {game.get('id')}, Date: {game.get('date')}, "
                ...
                if home_score == 0 and away_score == 0:
                    ...
                    if game_date:
                        try:
                            ...
                            console.print(
                                f"[yellow]Warning: Could not parse date {game_date}: {e}[/yellow]"
                            )
    ...
    def should_be_tbd(self, game: dict) -> bool:
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not parse date for game {game.get('id', 'unknown')}: {e}[/yellow]"
            )
    ...
    async def fix_game_status(self, game: dict, dry_run: bool = True) -> bool:
        ...
        update_data = {"home_score": None, "away_score": None}

        if dry_run:
            console.print(f"[yellow]DRY RUN:[/yellow] Would update game {game_id}:")
            console.print(
                f"  Current: home_score={game.get('home_score')}, away_score={game.get('away_score')}"
            )
            console.print("  New:     home_score=None, away_score=None")
            console.print(
                f"  Teams:   {game.get('home_team_name', 'Unknown')} vs {game.get('away_team_name', 'Unknown')}"
            )
        ...
        try:
            ...
            await self.client.update_score(str(game_id), update_data)

            console.print(
                f"[green]✓[/green] Updated game {game_id}: {game.get('home_team_name', 'Unknown')} vs {game.get('away_team_name', 'Unknown')}"
            )
            return True
    ...
    async def fix_all_placeholder_games(self, dry_run: bool = True) -> dict:
        ...
        console.print(
            f"[blue]Found {len(placeholder_games)} games with 0-0 scores marked as completed[/blue]"
        )
        ...
        try:
            if (
                not console.input(
                    f"\n[red]Are you sure you want to fix {len(placeholder_games)} games? (yes/no): [/red]"
                )
                .lower()
                .startswith("y")
            ):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fix placeholder scores in Supabase database"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        console.print(
            "[yellow]Please set MISSING_TABLE_API_BASE_URL and MISSING_TABLE_API_TOKEN environment variables[/yellow]"
        )
        return 1
    ...
    if dry_run:
        console.print(
            "[yellow]Running in DRY RUN mode. Use --execute to make actual changes.[/yellow]"
        )
    else:
    ...
    try:
        ...
        if dry_run and results["total"] > 0:
            ...
            console.print("  python scripts/fix-supabase-data.py --execute")
