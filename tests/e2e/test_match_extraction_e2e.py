...
class TestMatchExtractionE2E:
        ...
        async with get_browser_manager(browser_config) as browser_manager:
            async with browser_manager.get_page() as page:
                ...
                if not filters_applied:
                    logger.warning("Some filters may not have been applied successfully")
                ...
                if not browser_config.headless:
                    logger.info("Browser will remain open for 10 seconds for manual inspection...")
    ...
    async def test_match_extraction_with_different_filters(self, browser_config):
        ...
        async with get_browser_manager(browser_config) as browser_manager:
            for i, config in enumerate(test_cases):
                logger.info(f"Testing filter combination {i+1}: {config.age_group}, {config.division}")
                ...
                async with browser_manager.get_page() as page:
                    ...
                    logger.info(f"Filter combination {i+1} extracted {len(matches)} matches")
    ...
    async def test_match_extraction_performance(self, browser_config, test_config):
        ...
        async with get_browser_manager(browser_config) as browser_manager:
            async with browser_manager.get_page() as page:
                ...
                logger.info(f"Performance test: extracted {len(matches)} matches in {duration:.2f} seconds")
                ...
                if matches:
                    ...
                    logger.info(f"Performance: {matches_per_second:.2f} matches per second")

    async def _validate_and_display_matches(self, matches: list[Match], config: ScrapingConfig):
        """
        ...
        """
        logger.info(f"=== MATCH EXTRACTION RESULTS ===")
        logger.info(f"Configuration: {config.age_group}, {config.division}")
        logger.info(f"Total matches found: {len(matches)}")
        
        if not matches:
            logger.warning("No matches found - this could be normal if no games are scheduled")
            return
    ...
    async def _validate_match_structure(self, match: Match):
        ...
        assert match.status in ["scheduled", "in_progress", "completed"], f"Invalid status: {match.status}"
        
        # Teams should be different
        assert match.home_team != match.away_team, "Home and away teams should be different"
        
        # Score consistency
        if match.status == "completed":
            assert match.home_score is not None, "Completed match should have home score"
            assert match.away_score is not None, "Completed match should have away score"
    ...
    async def test_debug_match_extraction_step_by_step(self):
        ...
        async with get_browser_manager(browser_config) as browser_manager:
            async with browser_manager.get_page() as page:
                ...
                age_result = await filter_applicator.apply_age_group_filter(test_config.age_group)
                ...
                division_result = await filter_applicator.apply_division_filter(test_config.division)
                ...
                try:
                    date_result = await calendar_interactor.set_date_range_filter(start_date, end_date)
                ...
                for i, match in enumerate(matches):
                    logger.info(f"Match {i+1}: {match.home_team} vs {match.away_team} ({match.status})")
...
def run_visible_test():
    ...
    async def _run():
        ...
        test_config = ScrapingConfig(age_group="U14", division="Northeast", look_back_days=30)
