...
def handle_cli_error(e: Exception, verbose: bool = False):
    ...
    if "Connection refused" in error_message and "4318" in error_message:
        console.print("[yellow]⚠️  Metrics export failed (this is normal - metrics server not running)[/yellow]")
        if verbose:
        ...
        console.print("[red]❌ Operation timed out - the website may be slow or unavailable[/red]")
        if verbose:
            console.print(f"[dim]Full error: {error_message}[/dim]")
    else:
...
def display_statistics(matches: list[Match]):
    ...
    stats_table.add_row("📅 Scheduled", str(scheduled_matches), f"{scheduled_matches/total_matches*100:.0f}%")
    stats_table.add_row("✅ Completed", str(completed_matches), f"{completed_matches/total_matches*100:.0f}%")
    if in_progress_matches > 0:
        stats_table.add_row("🔄 In Progress", str(in_progress_matches), f"{in_progress_matches/total_matches*100:.0f}%")
    stats_table.add_row("⚽ With Scores", str(matches_with_scores), f"{matches_with_scores/total_matches*100:.0f}%")
    stats_table.add_row("🏟️  With Venues", str(matches_with_venues), f"{matches_with_venues/total_matches*100:.0f}%")
...
def interactive(
    ...
    age_group = Prompt.ask("Age group", default=DEFAULT_AGE_GROUP, choices=VALID_AGE_GROUPS)
...
@app.command()
def debug(
    step: Annotated[str, typer.Option("--step", "-s", help="Debug step: url, browser, navigate, consent, filters, extract, inspect")] = "all",
    headless: Annotated[bool, typer.Option("--headless/--no-headless", help="Run browser in headless mode")] = False,
    timeout: Annotated[int, typer.Option("--timeout", "-t", help="Browser timeout in seconds")] = 60,
):
    """
...
def test_url_accessibility():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            console.print(f"   ⚠️  HTTP status indicates potential issue: {response.status_code}")
...
async def test_browser_init(headless: bool, timeout: int):
    ...
    try:
        console.print(f"   🚀 Initializing browser (headless={headless}, timeout={timeout}s)")
    ...
    try:
        ...
        async with browser_manager.get_page() as page:
            ...
            for strategy in wait_strategies:
                ...
                if success:
                    console.print(f"   ✅ Navigation successful with '{strategy}' strategy")
    ...
    try:
        ...
        async with browser_manager.get_page() as page:
            ...
            if banner_present:
                ...
                console.print("   ℹ️  No consent banner detected (may have been handled already)")
...
async def test_filters(headless: bool, timeout: int):
    ...
    try:
        ...
        async with browser_manager.get_page() as page:
            ...
            console.print(f"   📋 Discovered filter options:")
            ...
            if options:
                ...
                console.print("   ⚠️  No filter options discovered - page structure may have changed")
...
async def test_extraction(headless: bool, timeout: int):
    ...
    try:
        ...
        async with browser_manager.get_page() as page:
            ...
            console.print(f"   📊 Extraction results:")
    ...
    try:
        ...
        async with browser_manager.get_page() as page:
            ...
            for i, select in enumerate(selects[:5]):  # Show first 5
                ...
                console.print(f"      {i+1}. name='{name}' id='{id_attr}' class='{classes}'")
            ...
            for keyword in filter_keywords:
                ...
                if elements:
                    console.print(f"      Found {len(elements)} elements containing '{keyword}'")
            
            # Check if page is still loading
            console.print("   ⏳ Checking if page is still loading...")
            
            loading_indicators = await page.query_selector_all(".loading, .spinner, [data-loading]")
            if loading_indicators:
                console.print(f"      ⚠️  Found {len(loading_indicators)} loading indicators - page may still be loading")
            else:
            ...
            console.print(f"   📋 After waiting: Found {len(selects_after)} select elements")
...
def inspect(
    headless: Annotated[bool, typer.Option("--headless/--no-headless", help="Run browser in headless mode")] = False,
    timeout: Annotated[int, typer.Option("--timeout", "-t", help="Browser timeout in seconds")] = 120,
):
    """
        ...
        async with browser_manager.get_page() as page:
            ...
            if success:
                ...
                popup_selectors = [
                    ...
                    ".cookie-banner", ".cookie-notice", ".gdpr-notice",
                    ".newsletter-popup", ".subscription-modal",
                    ".age-gate", ".location-selector",
                    # Bootstrap modals
                    ".modal.show", ".modal.fade.show",
                    # Common close button patterns
                    ".close", ".modal-close", "[aria-label*='close' i]",
                    "button[data-dismiss='modal']",
                    # MLS specific
                    ".mls-modal", ".mls-popup", ".mls-overlay"
                ]
                ...
                if found_popups:
                    console.print(f"⚠️  Found {len(found_popups)} visible pop-up/overlay elements:")
